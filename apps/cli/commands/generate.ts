...
export default createCommand("generate")
  ...
  .action(async (options) => {
     ...
     */
    totalProgress.increment();
    totalProgress.update({ title: "Universe collections".padEnd(TITLE_WIDTH) });
    globalProgress.update();
    ...
    for (const universeName of universeNames) {
      ...
      for (const regionName of regionNames) {
        ...
        for (const constellationName of constellationNames) {
          ...
          // @ts-expect-error
          universe.constellations[constellation.constellationID] =
            constellation;
          ...
          for (const solarSystemName of solarSystemNames) {
            ...
            Object.keys(solarSystem.stargates ?? {}).forEach((stargateID) => {
              ...
              universe.stargates[stargateID] =
                // @ts-expect-error
                solarSystem.stargates[stargateID];
              // @ts-expect-error
    ...
    await Promise.all(
      Object.keys(universe).map(async (_key) => {
        ...
        const progress = globalProgress.create(
          Object.keys(universe[key]).length,
          0,
          {
            title: `Generating ${key}`.padEnd(TITLE_WIDTH),
          },
        );
        globalProgress.update();
...
export async function generateCollectionFiles(
  ...
  const progress = globalProgress.create(0, 0, {
    title: `Generating ${file.outputPath}`.padEnd(TITLE_WIDTH),
  });
  globalProgress.update();
